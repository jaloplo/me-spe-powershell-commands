...
Try {
    Switch($PSCmdlet.ParameterSetName) {
        'All' {
            $Request = Invoke-MgGraphRequest -Method GET -Uri $('https://graph.microsoft.com/beta/storage/fileStorage/deletedContainers?$filter=containerTypeId eq ' + $ContainerTypeId) -ErrorAction Stop

            ConvertTo-Json -InputObject $Request
        }
        'One' {
            $Request = Invoke-MgGraphRequest -Method GET -Uri "https://graph.microsoft.com/beta/storage/fileStorage/deletedContainers/$ContainerId" -ErrorAction Stop
